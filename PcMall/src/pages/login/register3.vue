      ...
      <flow-state state="3" title="设置登录密码" is-done></flow-state>
            ...
            <div>
              <div class="pw-group">
                  <div class="setPaswd">
                      <span>请输入登录密码</span>
                      <Input v-model="password" ref="lgPaswd" type="password" placeholder="请输入您要设置的登录密码" :maxlength=16 />
                  </div>
                  <div class="password">
                      <span>请确认登录密码</span>
                      <Input v-model="confirmPassword" ref="rePaswd" type="password" placeholder="请再输入登录密码" />
                  </div>
...
export default {
  ...
  mounted: function () {
    if (this.$store.getters.authsuccess) {
      ...
      this.$router.replace({path: 'signup3'})
    }
  }
}
</script>
