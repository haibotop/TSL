  ...
  <div id="cbaOnsale">
    <Tabs type="card" value="step1" @on-click="initUed">
      <TabPane label="基本信息" name="step1">
        ...
        <Form :label-width="180" :model="baseInfo" :rules="baseInfoRules" ref="baseInfoForm">
          <FormItem label="商品名称：" prop="name">
            <Input class="my-input" @on-blur="judgeName" :maxlength="50" v-model.trim="name" disabled></Input>
        ...
        <Form :label-width="180" :model="publicParamsModel" ref="publicParamsForm" :rules="publicParamsRules">
          ...
          <FormItem :label="`${item.specName}：`" v-for="(item, index) in publicParams" :key="index" :prop="`common${index}`">
            ...
            <Select v-if="item.type === 3" v-model="item.value" class="my-select" :transfer="true" disabled>
      ...
      <TabPane label="销售参数" name="step3">
        ...
        <Form>
          <FormItem v-for="(item3,index3) in skuParams" :key="index3" :label="`${item3.specName}：`" class="row-cate"  v-if="item3.value.length !== 0">
          ...
          <FormItem>
            <div v-for="(item, index) in skuInput" class="diy-sku-box" :key="index">
              ...
              <div v-else>
                <Input class="diy-sku-input" :maxlength="20" v-model="item.name" size="small" disabled></Input>：
              </div>
              <div>
            ...
            <Button @click="getSkuGroups" :loading="skuGroupsLoading" type="primary" class="margin-left-10">获取SKU组合</Button>
            <RadioGroup v-model="priceWay">
              <Radio :label="2" style="margin: 0px 0px 0px 10px;" :disabled="true">金价</Radio>
              <Select v-model="goldPrice" :disabled="true" style="width: 100px;">
                <Option v-for="(item, index) in goldPrices" :key="index" :value="JSON.stringify(item)">{{item.name}}</Option>
              </Select>
              <Radio :label="1" style="margin: 0px 0px 0px 10px;" :disabled="true">一口价</Radio>
            </RadioGroup>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      saveToback () {
        ...
        for (let k in this.skuInput) {
          ...
          for (let l of this.skuInput[k].values) {
            if (this.skuInput[k].name === 'WeightOfGold&IsOnly1' && !/^\d+$/.test(l.spuSpecValue) && !/^\d+\.\d+$/.test(l.spuSpecValue) && l.spuSpecValue !== '') {
        ...
        for (let k of this.productInfo.spuInfos.spuSpec.select) {
          console.log(k.spuSpecValues[0].spuSpecValueId)
        ...
        if (this.commonAdding) {
          console.log(params)
          this.setSkuproduct(params, params.spuId)
        }
      },
      judgeName () {
      ...
      // 设置至少要选中一个sku属性
      hideSkuParm (key, value) {
        if (key.value.length === 1 && key.value[0] === value.cateSpuSpecValueName) {
          return true
        } else {
          return false
        }
      ...
      addDiySkuValues (index) {
        this.skuInput[index].values.push({spuSpecValueId: '', spuSpecValue: ''})
      },
      // 提交sku属性值，获得sku组合
      getSkuGroups () {
        ...
        for (let i of this.oldSkuInput) {
          ...
          for (let j of i.values) {
            let obja = {
              specValue: j.spuSpecValue,
              specValueId: j.spuSpecValueId
            }
            obj.specValues.push(obja)
          }
          param.oldGroup.push(obj)
        }
        // 新的参数
        ...
        for (let j of this.skuInput) {
          ...
          if (allNull) {
            this.$Message.error('销售属性 \'' + j.name.replace('WeightOfGold&IsOnly1', '重量(g)') + '\' 的值请至少填写一个！')
            return
          }
          if (obj.specName) {
            param.newGroup.push(obj)
        ...
        this.getSkuGroup((res) => {
          ...
          for (let i of res.skuInfos) {
            let flagOld = true
            for (let j of this.oldSkuGroups) {
              console.log('new:' + j.name.replace(this.name + ' ', ''), 'old:' + i.name)
              if (j.name.replace(this.name + ' ', '') === i.name) {
                flagOld = false
                let objN = j
                objN.name = j.name.replace(this.name + ' ', '')
                skuGroups.push(j)
                break
              }
            }
        ...
        }, param, this.productInfo.spuInfos.spuId)
      },
      // 设置默认SKU组合图集
      setDefaultImg () {
        let imgArr = []
      ...
      // --------------API--------------
      getSkuGroup (callback, param, spuId) {
        this.$http.post(...pdAPI.onGetSkuGroup(param, spuId))
        .then(response => {
      ...
      setSkuproduct (params, spuId) {
        // 保存多品修改信息
        this.$http.post(...pdAPI.onSetSkuInfo(params, spuId))
        .then(response => {
          if (response.data.code === 200) {
    ...
    mounted: function () {
      ...
      this.oldSkuParams = JSON.parse(JSON.stringify(arrSku))
      // sku销售属性
      let arraySku = []
      for (let j of this.productInfo.skuInfos.cateSelect) {
        if (!j.memo) {
      ...
      arraySku.sort((a, b) => {
        console.log(a)
        console.log(b)
        if (new Date(a.createDate) - new Date(b.createDate) > 0) {
          return 1
        } else {
          return -1
        }
      ...
      this.oldSkuInput = JSON.parse(JSON.stringify(arraySku))
      // sku显示table
      let skus = []
