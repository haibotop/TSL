      ...
      <TabPane label="基本信息" name="step1">
        <Form :label-width="180" :model="baseInfo" :rules="baseInfoRules" ref="baseInfoForm">
          ...
          <FormItem label="商品名称：" prop="name">
            <Input class="my-input" :maxlength="50" v-model="name"></Input>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    updated(){
      console.log('adverMes',this.adverMes)
    },
    data () {
      return {
        // ------------富文本
    ...
    methods: {
      ...
      saveToback () {
        ...
        if (this.price.length > 12) {
          this.$Message.warning('价格输入长度过大！')
          return
        } else if (!(/^(-?\d*)(\.\d+)?$/).test(this.price)) {
          this.$Message.warning('价格输入错误,请输出数字!')
          return
        }
      ...
      // API 保存单品信息
      setSpuproduct (param) {
        this.$http.post(...pdApi.setSpuInfo(param))
        .then(response => {
    ...
    computed: {
      adverMes: {
        ...
        set: function (val) {
          ...
          console.log('val',val)
          console.log('jhjhj',this.adverMesGet.spuSpecValue)
        }
      },
      location: {
        get: function () {
    ...
    mounted: function () {
      ...
      console.log('productInfo',this.productInfo.spuInfos.spuSpec.select[0].spuSpecValues[0].spuSpecValue)
      // 图集上传组件
