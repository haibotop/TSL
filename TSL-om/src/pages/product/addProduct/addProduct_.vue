      ...
      <TabPane label="销售属性" :disabled="currTab < 1">
        ...
        <Form inline class="margin-10" :model="noSku" :rules="noSkuRules" ref="noSkuForm">
          <FormItem label="软通售价：" :label-width="90" prop="price">
            <Input :disabled="skuGroupsParam1.length !== 0 || skuGroups.length !== 0" v-model="noSku.price"><span slot="prepend">￥</span></Input>
          </FormItem>
          <FormItem label="原价：" :label-width="60" prop="originalPrice">
            <Input :disabled="skuGroupsParam1.length !== 0 || skuGroups.length !== 0" v-model="noSku.originalPrice"><span slot="prepend">￥</span></Input>
          </FormItem>
          <FormItem label="展示库存(件)：" :label-width="110" prop="stock">
            <Input :disabled="skuGroupsParam1.length !== 0 || skuGroups.length !== 0" v-model="noSku.stock"></Input>
          </FormItem>
          <FormItem label="SKU编码：" :label-width="80" v-show="skuGroupsParam1.length !== 0 || skuGroups.length !== 0">
        ...
        <div class="my-btn-bar">
          <Button @click="back">上一步</Button><Button class="margin-left-10" @click="next" :loading="pAdding">发布</Button>
        </div>
      </TabPane>
    </Tabs>
  </div>
...
//          {name: '尺码', values: ['L', 'M', 'S']}
        ...
        noSkuRules: {
          ...
          originalPrice: [{required: true, message: '原价不可为空'}],
...
//          }
      ...
      addProduct () {
        ...
        if (this.skuGroups.length !== 0) {
          let skuGroups = JSON.parse(JSON.stringify(this.skuGroups))
          for (let i of skuGroups) {
            if (!this.isEmpty(i.price) && !this.isEmpty(i.stock) && !this.isEmpty(i.originalPrice)) {
              if (i.price !== null && i.stock !== null && i.originalPrice !== null) {
                skuInfos.push({
                  name: i.name,
          ...
          params.originalPrice = this.noSku.originalPrice
          params.price = this.noSku.price
          params.spuId = this.spuId
          params.stock = this.noSku.stock
          params.pictures = ['https://o5wwk8baw.qnssl.com/a42bdcc1178e62b4694c830f028db5c0/avatar']
