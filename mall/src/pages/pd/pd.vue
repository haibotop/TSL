<template>
  <div id="pd">
    <x-header v-show="!moreFlag" :left-options="leftOption">
      ...
      <div slot="right">
        <my-svg type="home" color="#fff" width="25" height="25" opacity="1" style="transform: translateY(-2px);" @click.native="$router.push({path: '/home'})"></my-svg>
      </div>
    </x-header>
    <!-- <x-header v-show="moreFlag" title="图文详情" :left-options="{backText: '', preventGoBack: true}" @on-click-back="moreFlag = false"></x-header>     -->
    ...
    <scroller ref="scroller" lock-x v-model="scrollerStatus" :bounce="false" height="-96" @on-scroll="onScroll">
      <div :style="`min-height:${scrollerHeight}px;`">
        <pdSwiper ref="pdSwiper" :data1="swiperData"></pdSwiper>
        <Group gutter="0px">
          <CellBox>
            <div style="width:100%">
              <div class="p-name-wrapper bkcolor">
              ...
              <div class="p-price-wrapper bkcolor">
                <div class="p-price">
                  <span v-if="skuInfo.hasDiscount" class="p-price">
                    <span>￥</span>{{skuInfo.sku ? skuInfo.sku.discountPrice : 0}}
                  </span>
                  <span v-else :class="skuInfo.hasDiscount ? 'discountPrice' : '' ">
                    <span>￥</span>{{skuInfo.sku ? handlePrice(skuInfo.sku.price) : 0}}
                  </span>
                </div>
              </div>
            </div>
          ...
          <Cell is-link v-show="specStr.length > 0" @click.native="openSpecModal('')">
            <div slot="title" class="my-cell-title" style="display: inline-block;">产品规格 {{specValueNameStr}}</div>
          </Cell>
          <Cell>
            <div slot="title" class="my-cell-title">运费</div>
            <div class="my-cell-title">免运费</div>
          </Cell>
          <Cell is-link @click.native="$router.push({path: '/deliverygoods'})">
            <div slot="title" class="my-cell-title">配送及退换</div>
          </Cell>
        ...
        <div class="more-popup" :style="moreStyle">
          ...
          <div v-show="tabIndex === 0" style="padding:10px 15px;">
          ...
          <div v-show="tabIndex === 1" style="padding:10px 15px;">
            <p class="introduction">{{introduction}}</p>
          </div>
          <div v-show="tabIndex === 2" style="padding:10px 15px;">
    ...
    <div class="sub-tab">
      ...
      <div class="shopping-cart" @click="goShoppingCart">
        ...
        <badge class="cart-badge" v-show="cartNum !== 0" :text="cartNum"></badge>
      </div>
      <div v-show="canBuy" class="collection" onclick="qimoChatClick();">
      ...
      <popup v-model="specFlag" height="75%" :popup-style="{'overflow-y': 'visible'}" is-transparent>
        <div slot="default" class="spec-popup bkcolor">
          <div class="skuInfo">
            ...
            <div class="sku-info">
              <div class="sku-info-price">
                <span v-if="skuInfo.hasDiscount">
                  <span>￥</span>{{skuInfo.sku ? skuInfo.sku.discountPrice : 0}}
                </span>
                <span v-else>
                  <span>￥</span>{{skuInfo.sku ? handlePrice(skuInfo.sku.price) : 0}}
          ...
          <scroller lock-x :height="specScrollerHeight">
            <div class="spec-wrapper">
              ...
              <group id="pd-popup-group" gutter="5px" class="spec-inner">
                ...
                <inline-x-number v-model="num" style="display:inline-block;float:right;" :min="1" width="50px"></inline-x-number>
              </group>
            </div>
          </scroller>
          <div class="spec-sub-tab" :style="bottomStyle">
            <!-- <div :class="cartBtnStyle" @click="postCartItem">加入购物袋</div><div :class="buyBtnStyle" @click="createOrder">直接购买</div> -->
            ...
            <div v-show="!disabledBuy && modalmodel === '1'" class="just-buy"  @click="createOrder">确定</div>
            <div v-show="!disabledBuy && modalmodel === '0'" class="just-buy"  @click="postCartItem">确定</div>
            <div v-show="disabledBuy" class="call-service" onclick="qimoChatClick();">联系客服</div>
          </div>
        </div>
      </popup>
